...
class IntroForm(npyscreen.Form):
    def create(self):
        ...
        self.add(
            ...
            name="Set the port number for the simulator",
            value="",
            editable=False,
        )
...
class MainMenu(npyscreen.FormBaseNew):
    def create(self):
        ...
        self.name = "Simulated PMAC"
        self.t2 = self.add(
            npyscreen.BoxTitle, name="Main Menu:", relx=2, max_width=24
        )  # , max_height=20)
        self.t3 = self.add(
            npyscreen.BoxTitle, name="Current Status:", rely=2, relx=26
        )  # , max_width=45, max_height=20)
...
class MyTCPHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        while 1:
            ...
            message = self.data[8:].decode("utf-8")
...
class CoordinateSystem:
    ...
    def run_program(self):
        ...
        self.delta_time = (
            self.controller.read_memory_address(buffer_memory_address) & 0xFFFFFF
        ) / 1000
            ...
            if status != 1:
                ...
                if current_time > (self.time_at_last_point + self.delta_time):
                    ...
                    if point_index == self.controller.get_m_var(current_buffer_fill):
                        ...
                        self.running_scan = False
                        self.in_position = 1
                        self.controller.set_m_var(
                            M_TRAJ_STATUS, 2
                        )  # Set status to IDLE
                    else:
                        # Work out delta time
                        buffer_memory_address = (
                            self.controller.get_m_var(current_buffer_address)
                            + point_index
                        )
                        self.delta_time = (
                            self.controller.read_memory_address(buffer_memory_address)
                            & 0xFFFFFF
                        ) / 1000
                        self.time_at_last_point = current_time
                        for axis in range(1, 9):
                            position_memory_address = buffer_memory_address + (
                                1000 * axis
                            )
                            current_position = self.controller.axes[axis].readPosition()
                            new_position = self.controller.read_position(
                                position_memory_address
                            )
                            if self.delta_time == 0:
                                logging.debug("Zero delta time for CS")
                                velocity = 1.0
...
class PMACSimulator:
    ...
    def parse(self, message):
        # Small function to pick out digits in Python3
        def getDigits(num):
            filt = filter(str.isdigit, num)
            tempStr = "".join(filt)
            return int(tempStr)
        ...
        try:
            ...
            for word in message.split():
                ...
                if "B" in word and "R" in word:
                    ...
                    # Search for the number
                    num = word[index:]
                    prog_no = getDigits(num)
                    self.cs[self.ccs].run_program()
                    self.pvars[4001] = 1
                    logging.debug(
                        "Execute motion program %d for CS %d", prog_no, self.ccs
                    )
                if "J=" in word:
                    index = word.find("J=")
                    # Search for the number
                ...
                if "M" in word:
                    ...
                    if writing:
                        ...
                        resp = str(self.mvars[getDigits(num)])
                if "CPU" in word:
                    resp = "DSP56321"
                elif "P" in word:
                    if "#" in word or word.strip() == "P":
                        ...
                        if writing:
                            ...
                            resp = str(self.pvars[getDigits(num)])
                        # resp = "0"
                if "CID" in word:
                    resp = "603382"
                elif "I" in word:
            ...
            logging.debug("Exception raised in parse loop")
            logging.debug("%s%", type(inst))
            resp = ""
...
if __name__ == "__main__":
    ...
    #                     level=logging.DEBUG)
    logging.basicConfig(
        filename="/tmp/pmac-simulator.log", filemode="w", level=logging.ERROR
    )
